 ...
 *
 * $barcode = new Barcode('展示的数据');
 * $barcode->draw('文件名 不传直接输出字节流');
 *
 *
 */
...
//		die;
	 ...
	 * @return bool
	 */
	public function draw($filename = null,Response $response) {
        ...
        $arr['_000'] = imagecolorallocate($img, 0, 0, 0);
        $arr['_fff'] = imagecolorallocate($img, 255, 255, 255);
		...
		for($i = 0; $i < count($bars); $i++) {
			imagefilledrectangle($img, $bars[$i][0], $bars[$i][1], $bars[$i][2], $bars[$i][3], $arr[$bars[$i][4]]);
		}
		...
		if($this->barcode_text) {
			...
			imagefilledrectangle($img, $this->barcode_padding, $this->barcode_height - $this->barcode_padding - $barcode_text_h,
				$bc_w - $this->barcode_padding, $this->barcode_height - $this->barcode_padding, $arr['_fff']);
